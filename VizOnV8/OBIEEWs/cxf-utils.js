...
// Is an element xsi:nil? Note, in IE this requires the use of the prefix 'xsi',
// literally.
...
function org_apache_cxf_getNodeText(node) {
	var r = "";
	//document.write("<br>required node name : "+node.nodeName+"<br>");
	if(node.nodeName == 'sawsoap:rowset')
	{
	//document.write("number of child nodes : "+node.childNodes.length);
	//document.write('<br>node sawsoap:rowset : '+node.nodeType+'<br>');
	}
...
function org_apache_cxf_pad_string(string, len, pad, type) {
	...
	return (type == org_apache_cxf_pad_string_PAD_LEFT ? append.concat(string)
...
//
var ORG_APACHE_CXF_XMLHTTPREQUEST_MS_PROGIDS = new Array("Msxml2.XMLHTTP.7.0",
...
function org_apache_cxf_getXMLHttpRequest() {
	...
	if (window.ActiveXObject != null) {
		...
		//
		for (var i = 0; i < ORG_APACHE_CXF_XMLHTTPREQUEST_MS_PROGIDS.length
				&& !success; i++) {
			try {
				httpRequest = new ActiveXObject(
						ORG_APACHE_CXF_XMLHTTPREQUEST_MS_PROGIDS[i]);
				success = true;
			} catch (ex) {
		...
		if (!success) {
			this.utils
					.trace("Unable to get any Microsoft XML HttpRequest object.");
...
function org_apache_cxf_parse_multipart_related() {
	...
	for (var px = 1; px < parts.length - 1; px++) {
		...
		var headers = [];
		nextHeaderLine: for (var endX = parttext.indexOf('\r', x); endX != -1; x = endX + 1, endX = parttext
				.indexOf('\r', x)) {
	...
	if (ready == 4) {
		...
		if (httpStatus == 200 || httpStatus == 0) {
			...
			if (this.onsuccess != null) {
				...
				this.onsuccess(this, this.req.responseText);
			}
		} else {
